...
params ["_trigger"];
INFO_1("PFH for %1 starting.",_trigger getVariable "grad_extracts_sectorName");
...
[{
    ...
    _oldOwner = _trigger getVariable "grad_extracts_currentOwner";
    _pps = _trigger getVariable "grad_extracts_pointsPerSecond";
    if (_pps > 0) then {
        _categoryName = format ["Held %1",_trigger getVariable "grad_extracts_sectorName"];
    ...
    _captureSides = _trigger getVariable "grad_extracts_captureSides";
    ...
    (_trigger getVariable "grad_extracts_sideStrenghts") params ["_strengthWest","_strengthEast","_strengthIndep"];
    ...
    _trigger setVariable ["grad_extracts_sideStrenghts",[_strengthWest,_strengthEast,_strengthIndep]];
    ...
    if (_newOwner != _oldOwner) then {
        _trigger setVariable ["grad_extracts_previousOwner",_oldOwner];
        _trigger setVariable ["grad_extracts_currentOwner",_newOwner];
        [_trigger] call grad_extracts_fnc_updateMarker;
        [_trigger] call grad_extracts_fnc_notification;

        _sectorName = _trigger getVariable "grad_extracts_sectorName";
        ...
        _points = _trigger getVariable "grad_extracts_pointsForCapture";
        [_newOwner,_points,_sectorName] call grad_points_fnc_addPoints;
        [_oldOwner,-_points,_sectorName] call grad_points_fnc_addPoints;

        [_trigger] call grad_extracts_fnc_updateTasks;

        if (_trigger getVariable "grad_extracts_lockAfterCapture") exitWith {[_handle] call CBA_fnc_removePerFrameHandler};
    };
